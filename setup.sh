...
for vicsek in "${train_vicsek[@]}"; do
    ...
    python3 obstacle_avoidance_sim.py --processes=$processes --batch-size=$batch_size --obstacles=0 --steps=60 --dt=0.3 --save-dir=../data/vicsek/${vicsek}_0 --boids=0 --vicseks=$vicsek --instances=50000 --prefix=train
done
...
for vicsek in "${valid_vicseks[@]}"; do
    ...
    python3 obstacle_avoidance_sim.py --processes=$processes --batch-size=$batch_size --obstacles=0 --steps=60 --dt=0.3 --save-dir=../data/vicsek/${vicsek}_0 --boids=0 --vicseks=$vicsek --instances=1000 --prefix=valid
done
